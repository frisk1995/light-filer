...
[filox.exe をダウンロード (v0.4.5)](https://github.com/frisk1995/light-filer/releases/latest/download/filox.exe)
...
- **検索フィルタ** — ツールバーの検索ボックスでファイル一覧をリアルタイム絞り込み
- **パスを開く** — Ctrl+L またはツールバーボタンでパスを直接入力してナビゲート
- **右クリックメニュー** — 開く / パスをコピー / ターミナルで開く / 名前の変更 / 削除 / 新規作成（フォルダ・テキストファイル）
- **インライン名前変更** — 右クリック → 名前の変更でその場で編集
- **削除確認ダイアログ** — 誤削除を防ぐ確認ステップ
- **日本語フォント内蔵** — Noto Sans JP をバイナリに同梱。OS を問わず日本語が正しく表示
...
| `NotoSansJP-Regular.ttf` | 日本語グリフ（バイナリに同梱） |
...
| ファイル検索 | ツールバーの検索ボックスに入力 |
| パスを開く | Ctrl+L またはツールバーの → ボタン |
